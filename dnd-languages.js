...
module.exports = {
    ...
    upload: function upload(req, res) {
        ...
        const files = fs.readdirSync('/server/dnd-languages/database/'+realJson.language+'/'+realJson.letter+'/')
        var letterExt = files.length + 1;
        decodeBase64Image('/server/dnd-languages/database/'+realJson.language+'/'+realJson.letter+'/Letter'+letterExt.toString()+".jpg", realJson.image.replace(/\ /g, '+'));
    ...
    getCount: function getCount(req, res) {
        ...
        var response = []
        const dir = fs.readdirSync('/server/dnd-languages/database/'+req.body.language+'/')
        for (const symbol of dir){
            const len = fs.readdirSync('/server/dnd-languages/database/'+req.body.language+'/'+symbol).length
            response.push(len)
        }
